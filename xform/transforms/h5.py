...
class H5transform(BaseTransform):
    ...
    ----------
    file :          str
                    Path to Hdf5 transformation file.
    ...
    def xform(self,
        ...
        with h5py.File(self.file, 'r') as h5:
            ...
            # Make sure we are within bounds
            # Note that we clip `mn` at 0 and `mx` at 2 at each end.
            # This is to make sure we have enough of the deformation field
            # to interpolate later on `offsets` even if all points are
            # outside the deformation field.
            mn = np.clip(mn, 0, np.array(self.shape[:-1][::-1]) - 2)
        ...
        if frac_out > 0.2:
            warnings.warn(f'A suspiciously a large fraction ({frac_out:.1%}) '
        ...
        if not affine_fallback:
            # Setting nan will fail if dtype is integer
            try:
                xf[is_out, :] = np.nan
            except ValueError:
